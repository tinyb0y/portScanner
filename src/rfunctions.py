            ...
            with open(results_xml, 'w') as f:
                ...
                while i < len(argv):
                    ...
                    if arg[0] == '-':
                        ...
                        name, value = arg.split('=', 1)
                        f.write(
                            '  <option type="module" name=%s>%s</option>\n' % (xmlquoteattr(name), xmlescape(value)))
                    i += 1
                f.write('</options>\n')
                f.write('<results>\n')
...
def build_logdir(opt_dir, opt_auto):
    if opt_auto:
        return create_time_dir(opt_dir or '/tmp/tinyb0y', opt_auto)
    elif opt_dir:
...
'''
        ...
        log_grp.add_option('-L', dest='auto_log', metavar='SFX',
                           help="automatically save into DIR/yyyy-mm-dd/hh:mm:ss_SFX (DIR defaults to '/tmp/tinyb0y')")
    ...
    def __init__(self, module, argv):
        ...
        logsvc = multiprocessing.Process(name='LogSvc', target=process_logs, args=(
        log_queue, module.Response.indicatorsfmt, argv, build_logdir(opts.log_dir, opts.auto_log)))
        logsvc.daemon = True
        logsvc.start()
    ...
    def start_threads(self):
        ...
        for num in range(self.num_threads):
            ...
            t = multiprocessing.Process(name='Consumer-%d' % num, target=self.consume,
